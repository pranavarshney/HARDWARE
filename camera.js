...
window.camera = {
    ...
    loadBase: function (raw) {
        ...
        set('cam-mp', raw.mp);
        set('cam-sensor', raw.mm);
        set('cam-readout', raw.readout);
        set('cam-mm', raw.baseMm);
        set('cam-f', raw.f);
        setCheck('cam-ois', raw.ois);
        set('cam-af', raw.af);
        set('cam-coating', raw.coating);
        set('cam-isp', raw.isp);
        set('cam-flash', raw.flash);
        set('cam-bits', raw.bits);
        set('cam-vid', raw.vid);
        set('cam-fps', raw.fps);
        set('cam-fmp', raw.fmp);
        set('cam-fap', raw.fap);
        ...
        if (raw.lenses) {
            this.lenses = JSON.parse(JSON.stringify(raw.lenses));
        }
    ...
    refreshLineup: function () {
        ...
        container.innerHTML = activeCams.map(cam => {
            ...
            <div class="panel" style="padding:15px; display:flex; flex-direction:column; gap:5px;">
                ...
                <div style="display:flex; gap:5px; margin-top:10px;">
                    <button onclick="window.cloneToArchitect(${cam.id})" 
